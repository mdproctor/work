package io.casehub.work.issuetracker.rls;
...
@ApplicationScoped
public class IssueTrackerRlsPolicyApplicator {

    private static final Logger LOG = Logger.getLogger(IssueTrackerRlsPolicyApplicator.class);

    static final List<String> TABLES = List.of("work_item_issue_link");
    ...
    void onStart(@Observes @Priority(101) StartupEvent ev) {
        ...
        LOG.infof("Applying RLS policies to %s tables", "issue-tracker");
        try (Connection conn = dataSource.getConnection();
            ...
            LOG.infof("RLS applied to %d %s tables", TABLES.size(), "issue-tracker");
        } catch (SQLException e) {
            throw new IllegalStateException("Failed to apply RLS policies to issue-tracker tables", e);
        }
    }
}
